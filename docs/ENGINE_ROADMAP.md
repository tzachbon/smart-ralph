...
| 5 | **CI snapshot separation** | `commands/implement.md` (coordinator prompt) | Add rule: executor must report task-level verification separately from global CI state (ruff/mypy). Task verification = what the task's Verify command checks. CI snapshot = overall project health. One cannot mask the other. |
...
| 5 | **CI snapshot tracking** | Separate from task verification: after each quality checkpoint task, record global CI state (ruff exit code, mypy exit code, coverage %) in .ralph-state.json. This is different from per-task verify results. |
