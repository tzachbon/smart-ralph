...
2. **Ralph Wiggum**: During execution (`/ralph-specum:implement`), the command invokes a `ralph-wiggum` skill from the Ralph Wiggum plugin. The coordinator prompt reads `.ralph-state.json`, delegates tasks to spec-executor via Task tool, and outputs `ALL_TASKS_COMPLETE` when done.
...
- `commands/implement.md` - Thin wrapper + coordinator prompt for Ralph Wiggum
- `commands/cancel.md` - Dual cleanup (cancel-ralph + state file deletion)
- `hooks/scripts/stop-watcher.sh` - Logging/validation watcher (Ralph Wiggum controls loop)
