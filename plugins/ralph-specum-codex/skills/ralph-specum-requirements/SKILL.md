...
8. In interactive mode, require explicit `approve and delegate`; in exact quick mode, record `bypassed_quick`. In both modes, run `phase_gate.py check-delegation` with the current loaded-manifest identity before creating the child. Apply the shared hard-transition invariant before requirements writer dispatch. A failed normal-mode `check-delegation` ends this invocation before phase transition, child dispatch, or target-artifact write; it does not create an alternate quick bypass.
