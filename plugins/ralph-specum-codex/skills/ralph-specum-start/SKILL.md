...
14. In interactive mode, handle canonical final choices first. At an active decision frontier, retain `classify-reply`; only otherwise may `phase_gate.py resolve-approval STATE --text TEXT` accept the one live `approve-and-delegate` action. An accepted result still runs `confirm --source approve-and-delegate`, then `check-delegation`, before creating a `research-analyst` child. In exact quick mode, record `bypassed_quick`. In both modes, run `phase_gate.py check-delegation` with the current loaded-manifest identity before creating a `research-analyst` child.
...
16. Validate `research.md`, merge `phase: "research"` and `awaitingApproval: true` in interactive mode or `false` in exact quick mode, update progress, and present artifact approval when interactive. Persist an `approvalGate` only when one current artifact or recorded-feedback revision action exists; this start handoff's artifact menu has multiple choices, so it creates no descriptor.
...
- Handle the listed canonical artifact choices first. Because this menu contains multiple choices, do not persist `approvalGate` and do not let a bare `yes` or other contextual affirmative choose among them. If later exactly one current artifact or recorded-feedback revision action exists, only `resolve-approval STATE --text TEXT` may select it; use the existing continuation or revision route and fresh writer checks, then clear or replace the descriptor only after success. On clarification, do not mutate state and ask one focused question.
- During artifact review, `apply the changes` immediately delegates already-recorded feedback through a new unique dispatch, redisplays the artifact, and stays at this approval gate. Ask one focused change question only when no feedback is pending. The helper alone appends the accepted reply's audit; chat history is never authorization state.
