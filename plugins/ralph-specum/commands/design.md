...
2. **If installed**: Run `mdview ./specs/$spec/design.md` using Bash to render the document beautifully in the terminal. This is especially useful for design docs which contain mermaid diagrams that mdview renders as ASCII art.
3. **If NOT installed**: Display this recommendation:
   ```
   Tip: Install mdview for beautiful terminal rendering of spec documents (including mermaid diagrams as ASCII art):
   curl -fsSL https://raw.githubusercontent.com/tzachbon/mdview/main/install.sh | sh

   Then view with: mdview ./specs/$spec/design.md
   ```
