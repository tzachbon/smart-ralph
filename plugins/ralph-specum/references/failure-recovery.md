...
**Verification Layer Bypass**: When a fix task completes (TASK_COMPLETE), the coordinator MUST NOT run the 3 verification layers. Fix tasks are intermediate — only the original task's completion triggers full verification. After fix task TASK_COMPLETE, proceed directly to "Retry Original Task".
