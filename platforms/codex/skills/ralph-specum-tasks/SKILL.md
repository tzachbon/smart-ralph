...
5. If quick mode is not active, use bundled grill-with-docs behavior before writing `tasks.md`. If `$grill-with-docs` exists, use it. Otherwise inspect code and docs inline, ask native questions one at a time, and capture stable terminology when useful.
...
- After the walkthrough, offer `continue to implementation`, `run review agent`, or `request changes`.
