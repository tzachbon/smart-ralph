...
| Quick mode | Broader shortcut aliases; literal authorization only | Exact `--quick` only | Preserves the current narrow bypass and all non-interview checks. |
...
| `plugins/ralph-specum-codex/skills/interview-framework-codex/references/algorithm.md` | Modify | Specify exact `--quick`, gate order, normal-mode stop behavior, and fresh-next-invocation recovery without changing helper semantics. |
...
- **Exact quick mode**: `--quick` creates only the existing `bypassed_quick` interview receipt; it still completes discovery, manifest loading, parent delegation, child receipt recording, and child write checks.
- **Fresh versus resumed recovery**: A partial interview can resume only while it remains the current valid interview flow. Once a normal-mode delegation check failed, a later explicit invocation creates a fresh identity rather than reusing the failed tuple.
...
- Only exact `--quick` bypasses questions and final approval, while discovery, manifest reload, `check-delegation`, receipt recording, and `check-agent-write` remain mandatory.
