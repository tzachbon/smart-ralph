...
- Normal or exact-quick failed-gate recovery after terminal interview state uses a fresh discovery revision and manifest/interview identity only on the next explicit invocation; a valid in-progress `collecting` or `awaiting_confirmation` interview that has not reached the boundary still resumes.
- Only exact `--quick` bypasses questions and final approval; discovery, fresh retry identity, manifest, parent delegation, load receipts, and `check-agent-write` remain required, and terminal `bypassed_quick` state is never reused after a failed quick gate.
- `tests/codex-phase-flow.bats` and unchanged `tests/phase-gates.bats` pass.
- The Codex manifest is `4.12.5`; the separate Claude marketplace remains package-owned.
...
- [x] 1.1 [RED] Add the hard-transition coordinator matrix regression
  - **Do**:
    ...
    2. Require the matrix to prove by wording and line order: failed `check-delegation` in either mode stops before state transition, child dispatch, or target-artifact write; terminal failed-gate recovery takes a fresh identity on the next explicit invocation; valid in-progress resumes remain valid; exact `--quick` retries rerun discovery and do not reuse terminal state while retaining all provenance and writer checks; and triage keeps `.epic-state.json` for every writer.
    3. Change the existing manifest assertion to require the planned Codex `4.12.5` version without coupling it to the separate Claude marketplace.
...
- [x] 1.2 [GREEN] Define the shared hard-transition invariant and fallback coverage
  - **Do**:
    ...
    2. In the algorithm, state the same fail-closed boundary, preserve the existing last pre-write `check-agent-write`, and define terminal fresh-next-explicit-invocation recovery for normal and exact-quick failures without changing helper semantics or quick authorization; keep only matching in-progress resumes valid.
...
- [x] 1.3 [GREEN] Apply the invariant to fresh, triage, and direct research dispatch
  - **Do**:
    1. Update the start contract so fresh and resumed research dispatch cannot run after a failed gate in either mode and the next explicit invocation creates a fresh identity after a terminal normal-mode or exact-quick failure.
...
- [x] 1.4 [GREEN] Apply the invariant to downstream phase dispatch
  - **Do**:
    ...
    2. Keep each existing child packet, unique dispatch identity, `check-agent-write` pre-write guard, artifact-approval behavior, prototype selection, and exact quick bypass unchanged while requiring fresh quick retry discovery and identity.
...
- [x] 1.5 [GREEN] Bump the Codex plugin patch version
  - **Do**:
    1. Change the Codex plugin manifest version from `4.12.4` to `4.12.5`.
  ...
  - **Done when**: The Codex manifest and its Bats regression assertion agree on exactly `4.12.5`.
  - **Verify**: `bats tests/codex-phase-flow.bats` exits 0.
  - **Commit**: `chore(codex): bump plugin version to 4.12.5`
...
- [x] 2.1 [VERIFY] Run the unchanged helper suite and the coordinator regression suite
  - **Do**:
    ...
    3. Confirm the matrix text contract proves the normal, direct, resumed, triage, exact-quick, and exact-quick-retry cases without adding a runtime enforcement mechanism.
