...
**POC Milestone**: By 1.34, `plugins/ralph-specum-codex/` exists with all required files, marketplace.json created, and `bats tests/codex-plugin.bats` passes basic structure checks.
...
- [x] 1.1 Create plugin directory skeleton and manifest
  ...
  - **Files**: `plugins/ralph-specum-codex/.codex-plugin/plugin.json`
  - **Done when**: `plugin.json` exists, is valid JSON, version is `4.9.1`, has `name`, `description`, `hooks` fields
  - **Verify**: `jq -e '.version == "4.9.1" and .name == "ralph-specum-codex" and (.hooks | has("Stop"))' plugins/ralph-specum-codex/.codex-plugin/plugin.json && echo PASS`
...
- [x] 1.2 Create marketplace.json with ralph-specum-codex entry
  ...
  - **Verify**: `jq -e '.[0].name == "ralph-specum-codex" and .[0].version == "4.9.1" and .[0].policy.installation == "AVAILABLE"' .agents/plugins/marketplace.json && echo PASS`
  - **Commit**: `feat(codex-plugin): create marketplace.json with ralph-specum-codex entry`
...
- [x] 1.4 [P] Copy Python scripts from platforms/codex with path fixes
  - **Do**:
    ...
    3. Copy `platforms/codex/skills/ralph-specum/scripts/count_tasks.py` to `plugins/ralph-specum-codex/scripts/`
    4. Search each file for any hardcoded `platforms/codex/` path references; update to `plugins/ralph-specum-codex/` if found
  - **Files**: `plugins/ralph-specum-codex/scripts/resolve_spec_paths.py`, `plugins/ralph-specum-codex/scripts/merge_state.py`, `plugins/ralph-specum-codex/scripts/count_tasks.py`
  - **Done when**: All 3 scripts present, no `platforms/codex` substring in any of them
  - **Verify**: `ls plugins/ralph-specum-codex/scripts/*.py | wc -l | grep -q 3 && ! grep -r 'platforms/codex' plugins/ralph-specum-codex/scripts/ && echo PASS`
...
- [x] 1.5 [P] Copy JSON schema
  ...
  - **Verify**: `jq . plugins/ralph-specum-codex/schemas/spec.schema.json > /dev/null && echo PASS`
...
- [x] 1.7 [P] Copy templates from Claude plugin (9 existing + epic.md)
  ...
  - **Files**: `plugins/ralph-specum-codex/templates/` (10 files)
  - **Done when**: All 10 template files exist in plugin templates dir
  - **Verify**: `ls plugins/ralph-specum-codex/templates/*.md | wc -l | grep -q 10 && echo PASS`
...
- [x] 1.8 [P] Copy bootstrap assets from platforms/codex
  ...
  - **Files**: `plugins/ralph-specum-codex/assets/bootstrap/AGENTS.md`, `plugins/ralph-specum-codex/assets/bootstrap/ralph-specum.local.md`
  - **Done when**: Both files exist
  - **Verify**: `[ -f plugins/ralph-specum-codex/assets/bootstrap/AGENTS.md ] && [ -f plugins/ralph-specum-codex/assets/bootstrap/ralph-specum.local.md ] && echo PASS`
...
- [x] 1.10 Copy and adapt reference files from platforms/codex
  - **Do**:
    ...
    3. Copy `platforms/codex/skills/ralph-specum/references/path-resolution.md` to `plugins/ralph-specum-codex/references/path-resolution.md`
    4. Copy `platforms/codex/skills/ralph-specum/references/parity-matrix.md` to `plugins/ralph-specum-codex/references/parity-matrix.md`
  ...
  - **Files**: `plugins/ralph-specum-codex/references/` (4 files)
  - **Done when**: All 4 references exist; parity-matrix has "Version Delta" section; workflow.md has both execution path sections
  - **Verify**: `grep -q "Version Delta" plugins/ralph-specum-codex/references/parity-matrix.md && grep -q "Hook-Driven" plugins/ralph-specum-codex/references/workflow.md && grep -q "Manual Fallback" plugins/ralph-specum-codex/references/workflow.md && echo PASS`
...
- [x] 1.12 Write stop-watcher.sh hook (Codex output format)
  ...
  - **Files**: `plugins/ralph-specum-codex/hooks/stop-watcher.sh`
  - **Done when**: File is executable; outputs `{"decision":"block",...}` JSON or exits 0; handles missing state file gracefully
  - **Verify**: `[ -x plugins/ralph-specum-codex/hooks/stop-watcher.sh ] && echo '{"cwd":"/tmp"}' | bash plugins/ralph-specum-codex/hooks/stop-watcher.sh; echo EXIT:$?`
...
- [x] 1.14 Write ralph-specum primary skill (bootstrap/help)
  - **Do**:
    ...
    3. Replace any `platforms/codex/` path references with `plugins/ralph-specum-codex/`
    4. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum/SKILL.md`
  - **Done when**: File exists; contains all required routing tokens; no `platforms/codex` refs; under 2000 words
  - **Verify**: `[ -f plugins/ralph-specum-codex/skills/ralph-specum/SKILL.md ] && grep -q "| Triage |" plugins/ralph-specum-codex/skills/ralph-specum/SKILL.md && grep -q "Response Handoff" plugins/ralph-specum-codex/skills/ralph-specum/SKILL.md && echo PASS`
...
- [x] 1.15 [P] Write ralph-specum-start skill (merges start+new)
  ...
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-start/SKILL.md`
  - **Done when**: File exists; contains `quick mode`, `granularity`, `awaitingApproval`, `spawn_agent`; no `Task tool` refs
  - **Verify**: `grep -q "quick mode" plugins/ralph-specum-codex/skills/ralph-specum-start/SKILL.md && grep -q "spawn_agent" plugins/ralph-specum-codex/skills/ralph-specum-start/SKILL.md && echo PASS`
...
- [x] 1.16 [P] Write ralph-specum-research skill
  ...
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-research/SKILL.md`
  - **Done when**: File exists; `spawn_agent` present; approval handoff tokens present
  - **Verify**: `grep -q "spawn_agent" plugins/ralph-specum-codex/skills/ralph-specum-research/SKILL.md && grep -q "approve current artifact" plugins/ralph-specum-codex/skills/ralph-specum-research/SKILL.md && echo PASS`
...
- [x] 1.18 [P] Write ralph-specum-requirements skill
  - **Do**:
    ...
    5. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-requirements/SKILL.md`
  - **Done when**: File exists; `spawn_agent("ralph-specum:product-manager"` present; approval tokens present
  - **Verify**: `grep -q 'ralph-specum:product-manager' plugins/ralph-specum-codex/skills/ralph-specum-requirements/SKILL.md && echo PASS`
...
- [x] 1.19 [P] Write ralph-specum-design skill
  - **Do**:
    ...
    5. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-design/SKILL.md`
  - **Done when**: File exists; `spawn_agent("ralph-specum:architect-reviewer"` present; approval tokens present
  - **Verify**: `grep -q 'ralph-specum:architect-reviewer' plugins/ralph-specum-codex/skills/ralph-specum-design/SKILL.md && echo PASS`
...
- [x] 1.20 [P] Write ralph-specum-tasks skill
  - **Do**:
    ...
    5. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-tasks/SKILL.md`
  - **Done when**: File exists; `spawn_agent("ralph-specum:task-planner"` present; granularity token present
  - **Verify**: `grep -q 'ralph-specum:task-planner' plugins/ralph-specum-codex/skills/ralph-specum-tasks/SKILL.md && grep -q 'granularity' plugins/ralph-specum-codex/skills/ralph-specum-tasks/SKILL.md && echo PASS`
...
- [x] 1.22 Write ralph-specum-implement skill (hook loop + manual fallback)
  ...
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-implement/SKILL.md`
  ...
  - **Verify**: `grep -q "Automatic Loop" plugins/ralph-specum-codex/skills/ralph-specum-implement/SKILL.md && grep -q "Manual Loop" plugins/ralph-specum-codex/skills/ralph-specum-implement/SKILL.md && grep -q "codex_hooks" plugins/ralph-specum-codex/skills/ralph-specum-implement/SKILL.md && echo PASS`
...
- [x] 1.23 [P] Write ralph-specum-cancel skill
  - **Do**:
    ...
    5. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-cancel/SKILL.md`
  - **Done when**: File exists; `.ralph-state.json` token present; no Task tool refs
  - **Verify**: `grep -q '.ralph-state.json' plugins/ralph-specum-codex/skills/ralph-specum-cancel/SKILL.md && ! grep -q 'Task tool' plugins/ralph-specum-codex/skills/ralph-specum-cancel/SKILL.md && echo PASS`
...
- [x] 1.24 [P] Write ralph-specum-status skill
  - **Do**:
    ...
    5. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-status/SKILL.md`
  - **Done when**: File exists; `.current-epic` and `approval state` tokens present
  - **Verify**: `grep -q '.current-epic' plugins/ralph-specum-codex/skills/ralph-specum-status/SKILL.md && grep -q 'approval state' plugins/ralph-specum-codex/skills/ralph-specum-status/SKILL.md && echo PASS`
...
- [x] 1.26 [P] Write ralph-specum-switch skill
  - **Do**:
    ...
    4. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-switch/SKILL.md`
  - **Done when**: File exists; `.current-spec` and `approval state` tokens present
  - **Verify**: `grep -q '.current-spec' plugins/ralph-specum-codex/skills/ralph-specum-switch/SKILL.md && echo PASS`
...
- [x] 1.27 [P] Write ralph-specum-triage skill
  - **Do**:
    ...
    5. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-triage/SKILL.md`
  - **Done when**: File exists; `spawn_agent("ralph-specum:triage-analyst"` present; epic tokens present
  - **Verify**: `grep -q 'ralph-specum:triage-analyst' plugins/ralph-specum-codex/skills/ralph-specum-triage/SKILL.md && grep -q 'specs/_epics' plugins/ralph-specum-codex/skills/ralph-specum-triage/SKILL.md && echo PASS`
...
- [x] 1.28 [P] Write ralph-specum-refactor skill
  - **Do**:
    ...
    4. Include approval handoff: "approve current artifact", "continue to implementation"
    5. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-refactor/SKILL.md`
  - **Done when**: File exists; approval handoff tokens present; no Task tool refs
  - **Verify**: `grep -q 'approve current artifact' plugins/ralph-specum-codex/skills/ralph-specum-refactor/SKILL.md && grep -q 'requirements.md' plugins/ralph-specum-codex/skills/ralph-specum-refactor/SKILL.md && echo PASS`
...
- [x] 1.30 [P] Write ralph-specum-index skill
  - **Do**:
    ...
    3. Remove any Task tool references
    4. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-index/SKILL.md`
  - **Done when**: File exists; `specs/.index` and `dry run` tokens present
  - **Verify**: `grep -q 'specs/.index' plugins/ralph-specum-codex/skills/ralph-specum-index/SKILL.md && grep -q 'dry run' plugins/ralph-specum-codex/skills/ralph-specum-index/SKILL.md && echo PASS`
...
- [x] 1.31 [P] Write ralph-specum-feedback skill
  ...
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-feedback/SKILL.md`
  - **Done when**: File exists; GitHub issue token present
  - **Verify**: `grep -q 'GitHub issue' plugins/ralph-specum-codex/skills/ralph-specum-feedback/SKILL.md && echo PASS`
...
- [x] 1.32 [P] Write ralph-specum-help skill
  - **Do**:
    ...
    4. Keep under 2000 words
  - **Files**: `plugins/ralph-specum-codex/skills/ralph-specum-help/SKILL.md`
  - **Done when**: File exists; `$ralph-specum-triage` and `Large effort flow` tokens present
  - **Verify**: `grep -q '\$ralph-specum-triage' plugins/ralph-specum-codex/skills/ralph-specum-help/SKILL.md && grep -q 'Large effort flow' plugins/ralph-specum-codex/skills/ralph-specum-help/SKILL.md && echo PASS`
...
- [x] 1.34 Write 9 agent-config TOML templates
  ...
  - **Files**: `plugins/ralph-specum-codex/agent-configs/README.md` + 9 `.toml.template` files
  - **Done when**: 9 template files exist + README; spec-executor template contains `TASK_COMPLETE`
  - **Verify**: `ls plugins/ralph-specum-codex/agent-configs/*.toml.template | wc -l | grep -q 9 && grep -q 'TASK_COMPLETE' plugins/ralph-specum-codex/agent-configs/spec-executor.toml.template && [ -f plugins/ralph-specum-codex/agent-configs/README.md ] && echo PASS`
...
- [x] 1.36 Write plugin README.md (installation + migration guide)
  - **Do**:
    ...
    3. Plugin structure must match `plugins/ralph-specum-codex/README.md` (AC-1.5)
  - **Files**: `plugins/ralph-specum-codex/README.md`
  - **Done when**: File exists; has "## Installation", "## Migration", "## Skills" sections; lists all 15 skill names
  - **Verify**: `grep -q "## Installation" plugins/ralph-specum-codex/README.md && grep -q "## Migration" plugins/ralph-specum-codex/README.md && grep -q "ralph-specum-implement" plugins/ralph-specum-codex/README.md && echo PASS`
...
- [x] 1.37 [VERIFY] POC Checkpoint: full plugin structure verification
  ...
  - **Verify**: `jq -e '.version == "4.9.1"' plugins/ralph-specum-codex/.codex-plugin/plugin.json > /dev/null && find plugins/ralph-specum-codex/skills -name 'SKILL.md' | wc -l | grep -q 15 && ls plugins/ralph-specum-codex/agent-configs/*.toml.template | wc -l | grep -q 9 && ls plugins/ralph-specum-codex/templates/*.md | wc -l | grep -q 10 && ls plugins/ralph-specum-codex/references/*.md | wc -l | grep -q 4 && ls plugins/ralph-specum-codex/scripts/*.py | wc -l | grep -q 3 && [ -x plugins/ralph-specum-codex/hooks/stop-watcher.sh ] && jq -e '.[0].name == "ralph-specum-codex"' .agents/plugins/marketplace.json > /dev/null && [ -f plugins/ralph-specum-codex/README.md ] && echo POC_COMPLETE`
...
- [x] 2.1 Audit skill word counts and trim any over-2000-word files
  ...
  - **Files**: Any `plugins/ralph-specum-codex/skills/*/SKILL.md` that exceed 2000 words
  - **Done when**: All 15 SKILL.md files are under 2000 words
  - **Verify**: `for f in $(find plugins/ralph-specum-codex/skills -name 'SKILL.md'); do words=$(wc -w < "$f"); [ "$words" -lt 2000 ] || { echo "OVER 2000: $f ($words words)"; exit 1; }; done && echo PASS`
...
- [x] 2.2 Audit parity matrix for completeness
  ...
  - **Files**: `plugins/ralph-specum-codex/references/parity-matrix.md`
  - **Done when**: Delta section has all 5 items; feature parity table present
  - **Verify**: `grep -q "epic.md" plugins/ralph-specum-codex/references/parity-matrix.md && grep -q "settings-template.md" plugins/ralph-specum-codex/references/parity-matrix.md && echo PASS`
...
- [x] 2.4 Remove any remaining platforms/codex path references from plugin files
  ...
  - **Done when**: Zero `platforms/codex` occurrences in `plugins/ralph-specum-codex/`
  - **Verify**: `! grep -r 'platforms/codex' plugins/ralph-specum-codex/ && echo PASS`
...
- [x] 2.5 Verify Python scripts still work with new paths
  - **Do**:
    ...
    3. Run `python3 plugins/ralph-specum-codex/scripts/resolve_spec_paths.py --help 2>&1 || true` and verify no syntax errors
  - **Files**: `plugins/ralph-specum-codex/scripts/*.py`
  - **Done when**: All 3 scripts parse without Python syntax errors
  - **Verify**: `python3 -m py_compile plugins/ralph-specum-codex/scripts/count_tasks.py && python3 -m py_compile plugins/ralph-specum-codex/scripts/merge_state.py && python3 -m py_compile plugins/ralph-specum-codex/scripts/resolve_spec_paths.py && echo PASS`
...
- [x] 3.3 Create tests/codex-plugin.bats (plugin structure tests)
  - **Do**:
    ...
    3. Write the following BATS tests:
       - `@test "codex plugin: plugin.json exists and is valid JSON"` — `jq . "$root/plugins/ralph-specum-codex/.codex-plugin/plugin.json" > /dev/null`
       ...
       - `@test "codex plugin: marketplace.json contains ralph-specum-codex entry"` — jq check
       ...
       - `@test "codex plugin: no platforms/codex references in plugin files"` — `! grep -r platforms/codex plugins/ralph-specum-codex/`
...
- [x] 3.5 Update tests/codex-platform.bats paths for new plugin location
  ...
  - **Done when**: No remaining `platforms/codex/skills/` strings in file; updated paths point to `plugins/ralph-specum-codex/`
...
- [x] 3.6 Rewrite codex-platform.bats tests for new plugin structure
  - **Do**:
    ...
    5. Rewrite `codex platform: docs describe the packaged distribution` test to check `plugins/ralph-specum-codex/README.md` instead of `platforms/codex/README.md` — verify the new README has installation and migration sections
  - **Files**: `tests/codex-platform.bats`
...
- [x] 3.9 Update .github/workflows/bats-tests.yml trigger paths
  ...
  - **Done when**: File contains `plugins/ralph-specum-codex/**` in trigger paths
  - **Verify**: `grep -q 'ralph-specum-codex' .github/workflows/bats-tests.yml && echo PASS`
...
- [x] V5 [VERIFY] AC checklist verification
  - **Do**: For each acceptance criterion, verify programmatically:
    ...
    - AC-3.1: `find plugins/ralph-specum-codex/skills -name 'SKILL.md' | wc -l` (must be 15)
    - AC-3.4: word count check on all SKILL.md
    - AC-5.1: `ls plugins/ralph-specum-codex/templates/*.md | wc -l` (must be 10)
    - AC-6.1: `ls plugins/ralph-specum-codex/agent-configs/*.toml.template | wc -l` (must be 9)
    - AC-6.4: `grep -q 'TASK_COMPLETE' plugins/ralph-specum-codex/agent-configs/spec-executor.toml.template`
    - AC-8.1: `[ -f plugins/ralph-specum-codex/hooks/stop-watcher.sh ]`
    - AC-10.1: `jq -e '.[0].name == "ralph-specum-codex"' .agents/plugins/marketplace.json`
...
- [x] 4.1 Create PR and push feature branch
  - **Do**:
    ...
    3. Create PR: title "feat(codex-plugin): create ralph-specum-codex plugin with full v4.9.1 parity", body summarizing: new plugin at `plugins/ralph-specum-codex/`, 15 skills, 9 agent templates, stop hook, marketplace entry, BATS tests, CI version-check
...
- [x] 5.3 Cleanup commit: delete platforms/codex/ (separate commit)
  - **Do**:
    ...
    3. Update `README.md` at repo root: remove any mention of `platforms/codex/` — replace with reference to `plugins/ralph-specum-codex/`
