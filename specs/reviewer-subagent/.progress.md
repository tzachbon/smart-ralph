...
- Pattern: All phase commands follow identical structure: Validate -> Interview -> Execute -> Review (spec-reviewer) -> Walkthrough -> Review & Feedback Loop -> Update State -> Commit -> Stop
- Pattern: Agent frontmatter must include name, description (with trigger phrases), model omitted (inherits parent)
- Pattern: Version bumps required in BOTH plugin.json AND marketplace.json (currently 3.5.0)
- Agent file convention: frontmatter (name, description with trigger phrases, model omitted (inherits parent)) + Core Philosophy mandatory block + When Invoked + Execution Flow + domain-specific sections + Output Format mandatory block + Communication Style mandatory block
...
- Commands:
  ...
  - grep -q "model omitted (inherits parent)" agents/spec-reviewer.md: PASS (exit 0)
  - grep -q "Artifact Review" commands/research.md: PASS (exit 0)
  - grep -q "iteration" commands/research.md: PASS (exit 0)
- Convention checks:
  - spec-reviewer.md frontmatter matches existing agent convention (name, description with trigger phrases, model omitted (inherits parent)): PASS
...
- Status: PASS
- Frontmatter checks (name, description, model omitted (inherits parent)): PASS
