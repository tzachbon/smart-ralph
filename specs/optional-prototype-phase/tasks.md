...
After all tasks are complete, the coordinator runs an independent implementation review and the Prototype Evidence Push Gate. Only after the gate permits and completes the feature-branch push may the coordinator create the PR, wait for required CI, resolve actionable human or automated review threads, and finish after a fresh GitHub query shows green required checks and zero unresolved actionable threads. A skipped or denied push is terminal for this workflow: record local completion, report `Remote lifecycle skipped: prototype evidence stayed local.`, and run no dependent PR creation, CI wait, review polling, issue write, or other remote lifecycle step. The coordinator must not merge or close the PR.
