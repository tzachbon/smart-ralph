...
- [x] 4.1 Resolve all in-scope review threads
  ...
  - **Verify**: `PR=$(gh pr view --json number --jq .number) && gh pr checks "$PR" && review_threads="$(gh api graphql --paginate -F owner=tzachbon -F name=smart-ralph -F number="$PR" -f query='query($owner:String!,$name:String!,$number:Int!,$endCursor:String){repository(owner:$owner,name:$name){pullRequest(number:$number){reviewThreads(first:100,after:$endCursor){nodes{isResolved} pageInfo{hasNextPage endCursor}}}}}' --jq '.data.repository.pullRequest.reviewThreads.nodes[] | select(.isResolved == false)')" && test -z "$review_threads"`
