...
- [x] 4.3 Create PR and verify CI - PR #95
- [x] 5.1 Monitor CI and fix failures - all green, no fixes needed
- [x] 5.2 Address code review comments - addressed CodeRabbit bot comments
- [x] 5.3 Final validation - all 6 criteria verified

## Current Task
All tasks complete

### Task 5.2: Address code review comments
- No CHANGES_REQUESTED reviews from human reviewers
- 6 inline comments from CodeRabbit bot (coderabbitai[bot]), state: COMMENTED
- Fixed 2 substantive code issues:
  1. implement.md: totalTasks increment now uses `$delta` variable instead of hardcoded +1 (SPLIT_TASK can insert N tasks)
  2. spec.schema.json: Added `required: ["id", "type"]` to modifications array items
- Fixed tasks.md version references: 3.5.0 -> 3.7.0 (matching actual version)
- Skipped: templates/tasks.md BAD example "check by hand" -- intentional anti-pattern in BAD example, doesn't trigger verification greps
- Skipped: design.md/research.md markdown lint (spec docs, not plugin code)

### Task 5.1: Monitor CI and fix failures
- CI status: All 3 checks passing (bats tests, .current-spec check, plugin version check)
- No failures to fix — CI was already green from task 4.3
- No commit needed (no code changes)

### Task 4.3: Create PR and verify CI
- Branch: feat/improve-task-generation (pushed to origin)
- PR: https://github.com/tzachbon/smart-ralph/pull/95
- CI status: All 4 checks passing (2x bats tests, .current-spec check, plugin version check)
- CodeRabbit review: pending (non-blocking bot review)

### Task 4.1: Bump plugin version
- Both files were already at 3.6.0 (bumped by another spec), so bumped to 3.7.0 instead of 3.5.0
- plugin.json: 3.6.0 -> 3.7.0
- marketplace.json: 3.6.0 -> 3.7.0
...
- Status: PASS - All 6 completion criteria met
- Criterion 1: All 60 tasks marked [x] in tasks.md -- PASS
- Criterion 2: CI all green (3/3 checks: bats tests, .current-spec, plugin version) -- PASS
- Criterion 3: No manual verification patterns: `! grep -qi "manual test" templates/tasks.md` -- PASS
- Criterion 4: Schema valid: `jq empty spec.schema.json` -- PASS
- Criterion 5: Version bumped to 3.7.0 -- PASS
- Criterion 6: All 7 files modified per design (+ bonus plan-synthesizer.md fix) -- PASS
- PR #95: https://github.com/tzachbon/smart-ralph/pull/95
...
- 7 files modified: templates/tasks.md, task-planner.md, spec-executor.md, implement.md, stop-watcher.sh, spec.schema.json, plan-synthesizer.md
...
- Version bump: task spec said 3.4.1->3.5.0 but actual version was already 3.6.0 (bumped by another spec), so bumped to 3.7.0 instead
...
### Verification: 4.2 [VERIFY] Full local validation: schema + grep checks + version
- Status: PASS
- Check 1: `jq empty plugins/ralph-specum/schemas/spec.schema.json` = valid JSON (exit 0) -- PASS
- Check 2: `! grep -qi "manual test" plugins/ralph-specum/templates/tasks.md` = no matches (exit 1 from grep, negated = success) -- PASS
- Check 3: `jq -r '.version' plugins/ralph-specum/.claude-plugin/plugin.json` = 3.7.0 (>= 3.5.0) -- PASS
- Check 4a: `grep -q "Task Sizing Rules" plugins/ralph-specum/agents/task-planner.md` = found -- PASS
- Check 4b: `grep -q "Task Modification Requests" plugins/ralph-specum/agents/spec-executor.md` = found -- PASS
- Check 4c: `grep -q "### 6e" plugins/ralph-specum/commands/implement.md` = found -- PASS
- All 6 validation checks passed, no fixes needed
